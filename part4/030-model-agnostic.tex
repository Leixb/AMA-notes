...
\section[Global measures]{Global measures of variable relevance}\index{global measures}
...
\begin{algorithm}{Variable importance by random permutations}{}\index{random permutations}
	\begin{enumerate}
...
\begin{theorem}{}{}\index{Shapley value}
...
It can be proven that the only explanation linear model $g$ verifying these three
...
\begin{definition}{Shapley kernel}{}
	...
	\end{equation*}
	\tcblower
	The \iemph{Shapley kernel} gives infinite weight to $z = 0_d$ and to $z = 1_d$,
...
This way of computing SHAP is known as \iemph{Kernel SHAP}.


\subsection{Break down plots}\index{break down plots}
...
The break-down contribution of the $j$-th coordinate of $x$ to the
...
in a \emph{break-down plot}, a horizontal bar chart (or \iemph{waterfall plot})
...
visualize them by waterfall plots (Gosiewska et al., 2020). % TODO: cite

\subsection{ICE: Individual conditional expectation, or ceteris paribus chart}
\index{ICE}\index{Individual conditional expectation}\index{ceteris paribus chart}
\index{local graphical IML}

\begin{definition}{Individual conditional expectation (ICE)}{}
    introduced by Goldstein et al. (2015) % TODO: cite
...
Given the prediction function $f(x),\,x \in \mathbb R^p$, the ICE of the $i$-th
...
there are no individual ALE curves. % TODO Cite Molnar 2019

Biecek and Burzykowski (2021) % TODO: cite
...
they define its \iemph{oscillation} as $\mathds E \left[ \left| f_j^{(i)}(X_j) - f(x) \right|\right]$
...
Additionally, they propose two \iemph{local agnostic plots}:
\begin{itemize}
